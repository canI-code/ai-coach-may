...
export default function ExamHistoryPage() {
  ...
  const handleRetake = async (sessionId: string) => {
    ...
    try {
      ...
      router.push(`/dashboard/${portalType}/practice/${data.sessionId}`);
    } catch (err: any) {
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Button 
            ...
            onClick={() => router.push(`/dashboard/${portalType}/practice`)} 
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="w-full bg-[#0a0a0b]/20 border border-white/5 rounded-xl p-4 md:p-6">
                              <div className="h-56 relative">
                                <LineChart 
                                  points={session.attempts.map(a => ({
                                    date: new Date(a.date),
                                    label: `Attempt #${a.attemptNumber}${a.isArchived ? ' *' : ''}`,
                                    value: a.scorePercentage
                                  }))} 
                                  accent="#f59e0b" 
                                  max={100}
                                  height={224}
                              ...
                              <div className="flex justify-between items-center text-[10px] text-[#a1a1aa] mt-2 border-t border-white/5 pt-2">
                                <span className="flex items-center gap-1">
                                  <span className="w-2 h-2 rounded-full bg-[#f59e0b] inline-block"></span> Active Server Quiz Attempts
                                </span>
                                <span className="flex items-center gap-1">
                                  <span className="w-2 h-2 rounded-full bg-amber-500/50 inline-block"></span> Archived Local Storage Attempts (*)
                                </span>
                              </div>
                            </div>
                          </div>
