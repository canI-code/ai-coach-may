...
export default function ProgressPage({ searchParams }: PageProps) {
  ...
  const [skillsTimescale, setSkillsTimescale] = useState<'week' | 'month' | 'year'>('month');
  ...
  const skillsSeries = useMemo(() => {
    ...
    return userInterests.map((skill, idx) => {
      ...
      // Group by timescale (week/month/year) to resolve multiple datapoints issue
      const groups: Record<string, { sum: number; count: number; date: Date }> = {};
      pts.forEach((p) => {
        let key = '';
        if (skillsTimescale === 'week') {
          const sunday = getStartOfWeek(p.date);
          key = getLocalDateString(sunday);
        } else if (skillsTimescale === 'month') {
          key = `${p.date.getFullYear()}-${String(p.date.getMonth() + 1).padStart(2, '0')}`;
        } else {
          key = `${p.date.getFullYear()}`;
        }
      ...
      const formattedPoints = Object.entries(groups).map(([key, g]) => {
        let label = '';
        if (skillsTimescale === 'week') {
          const [year, month, day] = key.split('-').map(Number);
          const d = new Date(year, month - 1, day);
      ...
      formattedPoints.sort((a, b) => a.date.getTime() - b.date.getTime());
    ...
    return (
      <div className="min-h-screen bg-transparent text-white p-0 relative">
        ...
        <div className="max-w-4xl mx-auto relative z-10 p-4 md:p-8">
          <div className="mb-4">
            <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/${portalType}/progress`)}>
          ...
          <div className="flex items-center justify-center min-h-[40vh]">
            <GlassCard className="max-w-md text-center border-amber-400/20" padding="lg">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button onClick={() => router.push(`/dashboard/${portalType}/practice`)} variant="secondary">
                  Practice Exam
                </Button>
              ...
              <GlassCard padding="lg" className="bg-[#0a0a0b]/40 border-white/5">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full md:w-auto">
                    ...
                    {/* Timescale selector */}
                    <div className="flex p-0.5 rounded-lg bg-white/5 border border-white/10 select-none shrink-0">
                      {TIMESCALES.map((ts) => (
                        <button
                          key={ts.value}
                          onClick={() => setSkillsTimescale(ts.value as any)}
                          className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all cursor-pointer ${
                            skillsTimescale === ts.value
