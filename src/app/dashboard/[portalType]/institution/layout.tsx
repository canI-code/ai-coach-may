...
import { 
  ...
  User, LogOut, BrainCircuit, Settings, Building2
} from 'lucide-react';
    ...
    <div className="flex min-h-screen bg-[#050506]">
      ...
      <div className="w-64 border-r border-white/5 bg-[#0a0a0b] flex-col h-screen sticky left-0 top-0 overflow-y-auto z-10 hidden md:flex shrink-0">
        <div className="p-6 flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-sm tracking-wide text-white leading-tight">Institution</div>
            <div className="text-xs text-purple-500/80">Dashboard</div>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${
                  active 
                  ? 'bg-[#18181b] text-purple-400 border border-white/5' 
                  : 'text-[#a1a1aa] hover:text-white hover:bg-white/5 border border-transparent'
                }`}
              >
                <item.icon size={18} className={active ? 'text-purple-400' : 'text-[#a1a1aa]'} />
                {item.name}
