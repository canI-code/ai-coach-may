...
export function LineChart({
  ...
  const cleanLabel = (str: string) => str.split('\u200B')[0];

  const processedSeries = useMemo<LineChartSeries[]>(() => {
    const baseSeries = series || (points ? [{ name: 'Overall', color: accent, points }] : []);
    
    // Check if any series has duplicate labels
    const hasDuplicates = baseSeries.some(s => {
    ...
    if (!hasDuplicates) return baseSeries;

    // If there are duplicate labels within a series, make them unique by appending a suffix
  ...
  const allLabels = useMemo(() => {
    ...
    processedSeries.forEach(s => s.points.forEach(addPt));
  ...
  // Empty state check
  const hasData = processedSeries.some(s => s.points && s.points.length > 0);
  if (!hasData) {
    return (
      <div
  ...
  }, [processedSeries, visibleLabels, plotBottom]);
    ...
    <div ref={containerRef} className={`w-full relative group/chart ${className}`}>
      ...
      >
        ...
        {visibleLabelIndices.map((idx) => {
          ...
          const cX = xOf(label);
          const text = truncate(cleanLabel(label), labelChars);
