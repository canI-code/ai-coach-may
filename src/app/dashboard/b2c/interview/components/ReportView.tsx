...
import { RadarChart } from '@/app/dashboard/b2c/components/charts/RadarChart';
import { LineChart } from '@/app/dashboard/b2c/components/charts/LineChart';
...
export function ReportView({ sessionId }: ReportViewProps) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'breakdown' | 'voice' | 'visual' | 'skill_gap' | 'per_question' | 'progress'>('overview');
    ...
    <div className="mx-auto w-full max-w-4xl space-y-8 py-8 animate-fade-in-up">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            iconPosition="left"
            onClick={() => router.push('/dashboard/b2c/interview-history')}
          >
            History
          </Button>
          <Button
          ...
          <Button
            ...
            iconPosition="left"
            onClick={() => router.push('/dashboard/b2c/interview')}
          >
            New Interview
          </Button>
        </div>
      ...
      {activeTab === 'voice' && (
        <div className="space-y-8 animate-fade-in">
          ...
          {/* Voice Timeline events */}
          <section className="rounded-3xl border border-white/5 bg-white/[0.03] p-6 shadow-md text-left">
            <h3 className="mb-4 text-lg font-semibold text-white">Voice & Tone events</h3>
            <div className="rounded-2xl bg-white/[0.01] border border-white/[0.02] p-4">
              {(() => {
                const voiceTimeline = report.behavioralTimeline.filter((entry) => {
                  const text = (entry.event || (entry as any).label || '').toLowerCase();
                  return text.includes('filler') || text.includes('voice') || text.includes('silent') || text.includes('silence') || text.includes('speech') || text.includes('pause') || text.includes('wpm') || text.includes('tone');
                });
                return <TimelineList entries={voiceTimeline} />;
              })()}
            </div>
          </section>
        </div>
      ...
      {activeTab === 'visual' && (
        <div className="space-y-8 animate-fade-in">
          ...
          <section className="rounded-3xl border border-white/5 bg-white/[0.03] p-6 shadow-md text-left">
            <h3 className="mb-4 text-lg font-semibold text-white">Visual & Expression events</h3>
            <div className="rounded-2xl bg-white/[0.01] border border-white/[0.02] p-4">
              {(() => {
                const visualTimeline = report.behavioralTimeline.filter((entry) => {
                  const text = (entry.event || (entry as any).label || '').toLowerCase();
                  return text.includes('eye') || text.includes('contact') || text.includes('gaze') || text.includes('head') || text.includes('look') || text.includes('face') || text.includes('expression') || text.includes('posture') || text.includes('movement') || text.includes('smile') || text.includes('emotion');
                });
                return <TimelineList entries={visualTimeline} />;
              })()}
            </div>
          </section>
        </div>
      ...
      {activeTab === 'skill_gap' && (
        <div className="space-y-8 animate-fade-in">
          {/* Strengths & Improvements Global Lists */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
            ...
            {/* Improvements Card */}
            <section className="rounded-3xl border border-amber-500/10 bg-amber-500/[0.02] p-6 shadow-md">
              <div className="mb-4 flex items-center gap-2 text-amber-400">
                <AlertTriangle size={18} />
                <h3 className="text-lg font-semibold text-white font-medium">Areas to Refine</h3>
              </div>
                ...
                {report.resources.map((resource) => (
                  <li key={resource.id}>
                    <a
                      ...
                      className="flex items-center justify-between gap-4 rounded-2xl border border-white/5 bg-white/[0.04] px-4 py-3.5 transition-colors hover:border-teal-400/30 hover:bg-white/[0.07]"
                    >
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        {resource.institutionCode && (
                          <span className="rounded-full border border-teal-400/20 bg-teal-400/10 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-teal-300">
                            Institution
                          </span>
                        )}
      ...
      {activeTab === 'progress' && (
        <div className="space-y-8 animate-fade-in">
          ...
          <section className="rounded-3xl border border-white/5 bg-white/[0.03] p-6 shadow-md text-left">
            <h3 className="mb-2 text-lg font-semibold text-white">Progress Trend</h3>
            <p className="text-xs text-gray-500 mb-6">Confidence Index score trend over recent sessions</p>
            
            {(() => {
              const trendPoints = (dashboardMetrics?.ciTrend || []).map((pt: any) => ({
                label: pt.label,
                value: pt.ciScore,
              }));
              
              return (
                <div className="w-full bg-white/[0.01] border border-white/[0.02] p-4 rounded-2xl">
                  <LineChart points={trendPoints} height={260} accent="#f59e0b" max={100} />
                </div>
              );
            })()}
          </section>
