                              ...
                              <div className="w-full bg-[#0a0a0b]/20 border border-white/5 rounded-xl p-4 md:p-6 overflow-x-auto">
                                <div className="min-w-[480px]">
                                  <svg viewBox="0 0 500 130" className="w-full h-auto overflow-visible">
                                    <defs>
                                      <linearGradient id={`line-grad-${group.sessionId}`} x1="0%" y1="0%" x2="100%" y2="0%">
                                      ...
                                      <linearGradient id={`area-grad-${group.sessionId}`} x1="0%" y1="0%" x2="0%" y2="100%">
                                    ...
                                    {(() => {
                                      ...
                                      const points = group.attempts.map((att, idx) => {
                                        const x = PaddingX + (N > 1 ? (idx * ActiveWidth) / (N - 1) : 0);
                                        const y = PaddingY + ActiveHeight - (att.ciScore * ActiveHeight) / 100;
                                        return { x, y, score: att.ciScore, num: att.attemptNumber };
                                      });
                                      ...
                                      return (
                                        <>
                                          <path d={areaPath} fill={`url(#area-grad-${group.sessionId})`} />

                                          <path 
                                            d={linePath} 
                                            stroke={`url(#line-grad-${group.sessionId})`} 
                                            strokeWidth="3.5" 
                                            fill="none" 
                                            strokeLinecap="round"
                                          ...
                                          {points.map((p, idx) => (
                                            <g key={idx} className="group/node">
                                              <circle 
                                                ...
                                                cy={p.y} 
                                                r="4.5" 
                                                className="fill-teal-400 stroke-[#0a0a0b] stroke-2 shadow-[0_0_8px_rgba(20,184,166,0.5)]" 
                                              />
                                    ...
                                    <div className="flex items-center gap-4">
                                      {isCompleted && (
                                        <Button
                                          ...
                                          size="sm"
                                          icon={<Eye size={14} />}
                                          onClick={() => router.push(`/dashboard/b2c/interview/${att.attemptId}/report`)}
                                      ...
                                      {isResumable && (
                                        <Button
                                          ...
                                          onClick={() => router.push(`/dashboard/b2c/interview/${att.attemptId}`)}
                                        >
                                          Resume
                                        </Button>
                                      )}
